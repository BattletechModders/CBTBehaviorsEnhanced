...
namespace CBTBehaviorsEnhanced {
    public static class HeatHelper {
        ...
        public static int NormalizedHeatSinkCapacity(this Mech mech) {
            ...
            Mod.Log.Trace($"Mech: {CombatantUtils.Label(mech)} has totalHeatSinkCap: {totalHeatSinkCap} from currentCap: {mech.HeatSinkCapacity} + usedCap: {mech.UsedHeatSinksCap()}");
        ...
        public static int NormalizedAdjustedHeatSinkCapacity(this Mech mech, bool isProjectedHeat, bool fractional) {
            ...
            Mod.Log.Trace($"Mech: {CombatantUtils.Label(mech)} has adjustedNormedCap: {adjustedNormalizedCap} from normedCap: {capacity} x multi: {mech.DesignMaskHeatMulti(isProjectedHeat)}");
        ...
        public static float DesignMaskHeatMulti(this Mech mech, bool isProjectedHeat) {
            ...
            try {
                ...
                if (isProjectedHeat) {
                    ...
                    if (mech.Pathing != null && mech.Pathing.CurrentPath != null && mech.Pathing.CurrentPath.Count > 0) {
                        ...
                        if (destinationDesignMaskDef != null && !Mathf.Approximately(destinationDesignMaskDef.heatSinkMultiplier, 1f)) {
                            Mod.Log.Trace($"Destination design mask: {destinationDesignMaskDef?.Description?.Name} has heatSinkMulti: x{destinationDesignMaskDef?.heatSinkMultiplier} ");
                ...
                Mod.Log.Error($"Failed to calculate designMaskHeatMulti due to error: {e}", e);
            }
            ...
            if (mech.Pathing.CurrentPath != null && mech.Pathing.CurrentPath.Count > 0) {
                ...
                foreach (MapTerrainCellWaypoint cell in terrainWaypoints) {
                    if (cell != null && cell.cell.BurningStrength > 0) {
                        Mod.Log.Trace($" --Adding {cell.cell.BurningStrength} heat from cell at worldPos: {cell.cell.WorldPos()}");
                        sumOfCellHeat += cell.cell.BurningStrength;
                        totalCells += 1;
                    }
                }
                terrainHeat = totalCells != 0 ? (float)Math.Ceiling(sumOfCellHeat / totalCells) : 0;
                Mod.Log.Trace($"TerrainHeat: {terrainHeat} = sumOfHeat: {sumOfCellHeat} / totalCells: {totalCells}");
            } else {
                ...
                if (cell != null && cell.BurningStrength > 0) {
                    Mod.Log.Trace($"Adding {cell.BurningStrength} heat from current position: {mech.CurrentPosition}");
